...
soap_flush_raw(struct soap *soap, const char *s, size_t n)
{
  ...
  {
    void *t;
    t = soap_push_block(soap, nullptr, n);
    if (!t)
...
soap_puthex(struct soap *soap, const unsigned char *s, int n)
{
  ...
  {
    soap->dom->text = soap_s2hex(soap, s, nullptr, n);
    if (!soap->dom->text)
      return soap->error;
    return SOAP_OK;
  }
...
soap_gethex(struct soap *soap, int *n)
{
  ...
  if ((soap->mode & SOAP_XML_DOM) && soap->dom)
  {
    soap->dom->text = soap_string_in(soap, 1, -1, -1, nullptr);
    return (unsigned char*)soap_hex2s(soap, soap->dom->text, nullptr, 0, n);
  }
#endif
#ifdef WITH_FAST
  soap->labidx = 0;
  ...
  for (;;)
  {
    ...
    for (i = 0; i < SOAP_BLKLEN; i++)
    {
      ...
      else
      {
        ...
        p = (unsigned char*)soap_save_block(soap, nullptr, nullptr, 0);
        return p;
      }
...
soap_putbase64(struct soap *soap, const unsigned char *s, int n)
{
  ...
  if ((soap->mode & SOAP_XML_DOM) && soap->dom)
  {
    soap->dom->text = soap_s2base64(soap, s, nullptr, n);
    if (!soap->dom->text)
      return soap->error;
    return SOAP_OK;
  }
...
soap_getbase64(struct soap *soap, int *n, int malloc_flag)
{
  ...
  {
    soap->dom->text = soap_string_in(soap, 1, -1, -1, nullptr);
    return (unsigned char*)soap_base642s(soap, soap->dom->text, nullptr, 0, n);
  }
#endif
#ifdef WITH_FAST
  soap->labidx = 0;
...
#else
  if (soap_alloc_block(soap) == nullptr)
    return nullptr;
  for (;;)
  {
    int i;
...
soap_xop_forward(struct soap *soap, unsigned char **ptr, int *size, char **id, char **type, char **options)
{
  ...
  {
    if (!soap_element_begin_in(soap, ":Include", 0, nullptr))
    {
...
{
  size_t i, k, n;
  if (!attr || !*attr || dim < 1) /* actually attr != nullptr and dim is guaranteed >= 1 by soapcpp2-generated code */
    return 0;
  i = strlen(attr);
  n = 1;
  do
  {
    for (; i > 0; i--)
      if (attr[i - 1] == '[' || attr[i - 1] == ',' || attr[i - 1] == ' ')
        break;
    n *= k = (size_t)soap_strtoul(attr + i, nullptr, 10);
    size[--dim] = (int)k;
...
#ifdef HAVE_RANDOM
      srandom((unsigned long)time(nullptr));
#else
...
#else
/* Alternative approach to check the key file for cert only when cafile==nullptr */
#ifndef WM_SECURE_KEY_STORAGE
  if (soap->password)
  {
    SSL_CTX_set_default_passwd_cb_userdata(soap->ctx, (void*)soap->password);
  ...
  if ((soap->ssl_flags & SOAP_SSL_RSA))
  {
    ...
    if (SSL_CTX_need_tmp_RSA(soap->ctx))
    {
      ...
      BIGNUM *bne = BN_new();
      RSA *rsa = RSA_new();
      if (!bne || !rsa || !BN_set_word(bne, e) || !RSA_generate_key_ex(rsa, SOAP_SSL_RSA_BITS, bne, nullptr) || !SSL_CTX_set_tmp_rsa(soap->ctx, rsa))
      {
        if (bne)
          BN_free(bne);
        if (rsa)
...
#if OPENSSL_VERSION_NUMBER >= 0x10002000L
      dh = DH_new();
      if (!DH_generate_parameters_ex(dh, n, 2/*or 5*/, nullptr))
      {
        DH_free(dh);
        return soap_set_receiver_error(soap, "SSL/TLS error", "Can't generate DH parameters", SOAP_SSL_ERROR);
      }
  ...
  if ((soap->ssl_flags & SOAP_SSL_CLIENT))
  {
    ...
    else
    {
      ...
      ret = gnutls_priority_set_direct(soap->session, "PERFORMANCE:+ANON-DH:!ARCFOUR-128", nullptr);
      if (ret != GNUTLS_E_SUCCESS)
        return soap_set_receiver_error(soap, soap_ssl_error(soap, ret, 0), "SSL/TLS set priority error", SOAP_SSL_ERROR);
  ...
  else
  {
    ...
    if (gnutls_priority_set_direct(soap->session, soap->tmpbuf, nullptr) != GNUTLS_E_SUCCESS)
...
#endif
#ifdef WITH_SYSTEMSSL
  gsk_iocallback local_io = { ssl_recv, ssl_send, nullptr, nullptr, nullptr, nullptr };
  int retries, r, s;
  if (!soap_valid_socket(sk))
    return soap_set_receiver_error(soap, "SSL/TLS error", "No socket in soap_ssl_accept()", SOAP_SSL_ERROR);
  soap->ssl_flags &= ~SOAP_SSL_CLIENT;
...
again:
  ...
  if (soap->client_addr)
  {
    ...
    if (inet_pton(AF_INET, soap->client_addr, (void*)&addr.sin_addr) != 1 || bind(sk, (struct sockaddr*)&addr, sizeof(addr)))
    {
      ...
      (void)soap->fclosesocket(soap, sk);
      soap->client_addr = nullptr;
      soap->client_port = -1;
      return soap->socket = SOAP_INVALID_SOCKET;
    }
    soap->client_addr = nullptr; /* disable bind before connect, so need to set it again before the next connect */
  ...
  if (soap->client_interface)
  {
    if (inet_pton(AF_INET, soap->client_interface, &soap->peer.in.sin_addr) != 1)
    {
      ...
      (void)soap->fclosesocket(soap, sk);
      soap->client_interface = nullptr;
      return soap->socket = SOAP_INVALID_SOCKET;
    }
    soap->client_interface = nullptr; /* disable client interface, so need to set it again before the next connect */
  }
#endif
  soap->errmode = 2;
  if (soap->proxy_host)
  ...
  if (soap->client_addr)
  {
    ...
    soap->client_addr = nullptr; /* disable bind before connect, so need to set it again before the next connect */
    soap->client_addr_ipv6 = nullptr; /* disable bind before connect, so need to set it again before the next connect */
    soap->client_port = -1; /* disable bind before connect, so need to set it again before the next connect */
  }
  else
#endif
  ...
  if (endpoint && !soap_tag_cmp(endpoint, "https:*"))
  {
    ...
    if (soap->proxy_host)
    {
      ...
      soap->error = soap->fpost(soap, endpoint, host, port, nullptr, nullptr, 0);
          ...
          for (i = 0; i < ext_count; i++)
          {
            ...
            if (ext_str && !strcmp(ext_str, "subjectAltName"))
            {
              ...
              if (data)
              {
                ...
                {
#if OPENSSL_VERSION_NUMBER > 0x0090800fL
                  ext_data = meth->d2i(nullptr, (const unsigned char **)&data, ext->value->length);
#else
  ...
  if (soap->ssl)
  {
    ...
    if (soap_valid_socket(soap->socket))
    {
      ...
      if (r == 0 && !soap->fshutdownsocket(soap, soap->socket, SOAP_SHUT_WR))
      {
        ...
        if (!SSL_want_read(soap->ssl) || SSL_read(soap->ssl, nullptr, 0))
        {
          r = SSL_shutdown(soap->ssl);
          if (r <= 0)
          {
...
soap_accept(struct soap *soap)
{
  ...
  for (;;)
  {
    ...
    if (soap_valid_socket(soap->socket))
    {
      ...
      soap->ip6[2] = 0;
      soap->ip6[3] = 0;
      if (getaddrinfo(soap->host, nullptr, &hints, &res) == 0 && res)
      {
...
#endif
#ifndef WITH_LEANER
  soap->fsvalidate = nullptr;
  soap->fwvalidate = nullptr;
...
http_parse(struct soap *soap)
{
  ...
  *soap->endpoint = '\0';
  ...
  soap->cors_origin = nullptr;
  soap->cors_method = nullptr;
  soap->cors_header = nullptr;
  do
  {
    soap->length = 0;
...
http_parse_header(struct soap *soap, const char *key, const char *val)
{
  ...
  {
    soap->length = soap_strtoull(val, nullptr, 10);
    if (soap->length == 0)
      soap->body = 0;
  }
  ...
  else if (!soap_tag_cmp(key, "Expect"))
  {
    ...
    {
      if ((soap->error = soap->fposthdr(soap, "HTTP/1.1 100 Continue", nullptr)) != SOAP_OK
       || (soap->error = soap->fposthdr(soap, nullptr, nullptr)) != SOAP_OK)
        return soap->error;
    }
  }
#endif
...
http_response(struct soap *soap, int status, ULONG64 count)
{
  ...
  soap->cors_origin = nullptr;
  soap->cors_methods = nullptr;
  soap->cors_headers = nullptr;
  ...
  for (p = soap->cookies; p; p = p->next)
  {
     ...
     ) && p->name && p->value && *p->name && *p->value)
    {
      ...
      *s++ = '=';
      ...
      {
        time_t n = time(nullptr) + p->maxage;
        struct tm T, *pT = &T;
        size_t l = 0;
...
soap_putcookies(struct soap *soap, const char *domain, const char *path, int secure)
{
  ...
  while ((q = *p))
  {
    ...
    else
    {
      ...
      {
        struct hostent hostent;
        if (!tcp_gethostbyname(soap, (char*)domain, &hostent, nullptr))
        {
         ...
         )
      {
        ...
        DBGLOG(TEST, SOAP_MESSAGE(fdebug, "Sending cookie %s=%s path=\"/%s\" domain=\"%s\"\n", q->name ? q->name : "(nullptr)", q->value ? q->value : "(null)", q->path ? q->path : "(null)", q->domain ? q->domain : "(null)"));
...
{
  struct soap_cookie *p = nullptr, *q = nullptr;
  const char *s = val;
  ...
  while (*s)
  {
    ...
    {
      s = soap_decode_val(tmp, sizeof(tmp), s);
      p->expire = (ULONG64)(now + soap_strtol(tmp, nullptr, 10));
    }
...
soap_init_send(struct soap *soap)
{
  ...
  *soap->tag = '\0';
  ...
  soap->mode &= ~SOAP_IO_LENGTH;
  if ((soap->mode & SOAP_IO) == SOAP_IO_STORE && soap_alloc_block(soap) == nullptr)
    return soap->error;
...
#endif
#ifndef WITH_LEAN
  soap->start = (ULONG64)time(nullptr);
#endif
  return SOAP_OK;
}
...
soap_embedded_id(struct soap *soap, int id, const void *p, int t)
{
  ...
  if (id < -1)
    return soap_embed(soap, p, nullptr, 0, t);
  if (id < 0)
  {
    id = soap_pointer_lookup(soap, p, t, &pp);
...
soap_attachment(struct soap *soap, const char *tag, int id, const void *p, const void *a, int n, const char *aid, const char *atype, const char *aoptions, const char *type, int t)
{
  ...
  if (!p || !a || (!aid && !atype) || (!soap->encodingStyle && !(soap->omode & (SOAP_ENC_DIME | SOAP_ENC_MIME | SOAP_ENC_MTOM | SOAP_XML_GRAPH))) || (soap->omode & SOAP_XML_TREE))
    return soap_element_id(soap, tag, id, p, a, n, type, t, nullptr);
...
soap_delete(struct soap *soap, void *p)
{
  ...
  soap->fault = nullptr; /* assume this was deallocated */
  soap->header = nullptr; /* assume this was deallocated */
}
...
soap_end_send_flush(struct soap *soap)
{
  ...
  if ((soap->mode & SOAP_IO)) /* need to flush the remaining data in buffer */
  {
    ...
    if ((soap->mode & SOAP_IO) == SOAP_IO_STORE)
    {
      ...
      if (soap->os)
      {
        char *b = (char*)soap_push_block(soap, nullptr, 1);
        if (b)
        {
          *b = '\0';
